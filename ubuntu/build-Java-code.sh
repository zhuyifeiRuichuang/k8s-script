...
# 用于打包Java代码，需自定义代码所在目录，自定义JDK版本，自定义maven版本。自动打包代码，并将生成的jar文件自动存档在当前目录。
# 需使用最新版docker。机器需联网。
...
docker run -i \
    ...
    "$JDK_IMAGE" \
    /bin/bash -c "
        echo '安装基础工具（含apt-utils，消除配置警告）...'
        # 新增apt-utils解决debconf警告
        apt-get update >/dev/null && apt-get install -y apt-utils wget tar >/dev/null
